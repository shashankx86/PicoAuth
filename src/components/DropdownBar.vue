<template>
  ...
  export default {
    ...
    methods: {
      ...
      setDefaultService() {
        if (this.config && Object.keys(this.config).length > 0 && !this.config[this.selectedService]) {
    ...
    watch: {
      config: {
        ...
        immediate: true // Ensure the handler runs immediately when config is updated
      }
    },
    mounted() {
      this.setDefaultService();
